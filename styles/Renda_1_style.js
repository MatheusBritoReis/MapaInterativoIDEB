/* 
 * RENDA — classificação por desvio padrão
...
var style_Renda_1_stats = null;

function get_style_Renda_1_stats() {
    if (style_Renda_1_stats) return style_Renda_1_stats;
    ...
    var values = features
        .map(function(f) { return Number(f.get('Renda')); })
        .filter(function(v) { return Number.isFinite(v); });

    if (!values.length) {
        style_Renda_1_stats = {mean: 0, sd: 0, n: 0};
        return style_Renda_1_stats;
    }
...
var style_Renda_1 = function(feature, resolution) {
    ...
    var labelText = "";
    var value = Number(feature.get("Renda"));
    ...
    var stats = get_style_Renda_1_stats();
    var mean = stats.mean;
    var sd = stats.sd;
