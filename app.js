...
var state={etapa:'',rede:'',municipio:'',indicator:'port',renda:false,ppi:false,municipios:true,tab:'1',contextAxis:'renda'};
var CONTEXT_RADIUS_METERS=3000;
var ppiContextThresholds={low:null,high:null};
var analysisCache={};
...
var PPI_CLASSES=[
 {max:0,color:'#f1f5f9',label:'Sem população registrada'},
 {max:25,color:'#eff6ff',label:'Concentração muito baixa'},
 {max:50,color:'#dbeafe',label:'Concentração baixa'},
 {max:100,color:'#bfdbfe',label:'Concentração moderada'},
 {max:150,color:'#93c5fd',label:'Concentração moderada-alta'},
 {max:250,color:'#60a5fa',label:'Concentração alta'},
 {max:350,color:'#3b82f6',label:'Concentração alta'},
 {max:500,color:'#2563eb',label:'Concentração muito alta'},
 {max:750,color:'#1d4ed8',label:'Concentração muito alta'},
 {max:Infinity,color:'#1e3a8a',label:'Concentração muito alta'}
];
...
function visibleSchoolFeature(f){var p=f.getProperties();return !!state.etapa && norm(p.ETAPA)===state.etapa && (!state.rede||norm(p.REDE)===state.rede) && (!state.municipio||norm(p.NO_MUNICIPIO)===state.municipio)}
...
function loadSetores(){
 ...
 setoresPromise=fetch('./layers/setores_renda_ppi.geojson').then(function(r){if(!r.ok)throw new Error('Não foi possível carregar a base territorial.');return r.json()}).then(function(data){
   var fs=new ol.format.GeoJSON().readFeatures(data,{dataProjection:'EPSG:4326',featureProjection:'EPSG:3857'});
   var ppis=fs.map(function(f){return num(f.get('PPI'))}).filter(function(v){return Number.isFinite(v)});
   if(ppis.length>=3){ppiContextThresholds.low=percentile(ppis,0.33);ppiContextThresholds.high=percentile(ppis,0.66);}
   setorSource=new ol.source.Vector({features:fs});
   setorRendaLayer=new ol.layer.Vector({source:setorSource,visible:false,zIndex:5,style:function(f){var c=classify(num(f.get('Renda')),RENDA_CLASSES);return sectorStyle(c?c.color:'rgba(226,232,240,.45)')}});
   setorPpiLayer=new ol.layer.Vector({source:setorSource,visible:false,zIndex:5,style:function(f){var c=classify(num(f.get('PPI')),PPI_CLASSES);return sectorStyle(c?c.color:'rgba(226,232,240,.45)')}});
   map.addLayer(setorRendaLayer);map.addLayer(setorPpiLayer);return setorSource;
 }).catch(function(e){setoresPromise=null;throw e});
}
function schoolContext(f){
 if(!setorSource||!f)return null;
 var point=f.getGeometry().getCoordinates();
 var r=CONTEXT_RADIUS_METERS;
 var extent=[point[0]-r,point[1]-r,point[0]+r,point[1]+r];
 var candidates=setorSource.getFeaturesInExtent(extent),near=[];
 for(var i=0;i<candidates.length;i++){
   var g=candidates[i].getGeometry();
   if(!g)continue;
   var closest=g.getClosestPoint(point),dx=closest[0]-point[0],dy=closest[1]-point[1];
   if(Math.sqrt(dx*dx+dy*dy)<=r){
     var p=candidates[i].getProperties();
     near.push({feature:candidates[i],renda:num(p.Renda),ppi:num(p.PPI),municipio:norm(p.NM_MUN),codigo:norm(p.CD_SETOR)});
   }
 }
 if(!near.length)return null;
 var rendas=near.map(function(x){return x.renda}),ppis=near.map(function(x){return x.ppi});
 return {feature:near[0].feature,renda:mean(rendas),ppi:mean(ppis),municipio:norm(f.get('NO_MUNICIPIO'))||norm(near[0].municipio),codigo:norm(near[0].codigo),sectors:near.length,radius:CONTEXT_RADIUS_METERS};
}
...
function updateMapLegend(){var el=$('map-legend'),active=state.renda?'renda':state.ppi?'ppi':null;if(!active){el.classList.remove('visible');return}var classes=active==='renda'?RENDA_CLASSES:PPI_CLASSES;el.innerHTML='<div class="legend-title">'+(active==='renda'?'Renda média dos setores':'Concentração de população PPI')+'</div><div class="legend-sub">Clique em um setor para consultar os valores exatos.</div>'+classes.map(function(c){return '<div class="legend-row"><span style="width:22px;height:12px;border-radius:3px;background:'+c.color+';display:inline-block;border:1px solid #cbd5e1"></span>'+esc(c.label)+'</div>'}).join('');el.classList.add('visible')}

function ppiContextLabel(v){
 if(v===null)return 'Sem informação';
 if(ppiContextThresholds.low!==null && v>=ppiContextThresholds.high)return 'Alta concentração de população PPI';
 if(ppiContextThresholds.low!==null && v>=ppiContextThresholds.low)return 'Concentração intermediária de população PPI';
 return 'Baixa concentração de população PPI';
}
function prepareContextThresholds(){
 var vals=schoolFeatures.map(function(f){var c=analysisCache[schoolKey(f)];return c?c.ppi:null}).filter(function(v){return Number.isFinite(v)});
 if(vals.length>=3){ppiContextThresholds.low=percentile(vals,0.33);ppiContextThresholds.high=percentile(vals,0.66);}
}
...
function renderPhase1(){
 ...
 if(ctx){html+='<div class="section-title">Perfil socioeconômico territorial</div><div class="cards">'+card('Renda média no raio de 3 km',money(ctx.renda),'Média dos setores encontrados no entorno')+card('Perfil PPI no raio de 3 km',ppiContextLabel(ctx.ppi),'Média da população PPI dos setores no entorno')+'</div><div class="info">Os valores territoriais são médias dos setores encontrados em um raio de 3 km da escola; eles descrevem o entorno e não representam características individuais dos estudantes.</div>'}else html+='<div class="callout amber">O setor censitário ainda não pôde ser associado a esta escola.</div>';
 return html;
}
function renderPhase2(){
 ...
 if(c)html+='<div class="cards">'+card('Renda no raio de 3 km',money(c.renda),'Média dos setores no entorno')+card('Perfil PPI no raio de 3 km',ppiContextLabel(c.ppi),'Concentração média no entorno')+'</div>';
 html+='<div class="section-title">Comparação com escolas de contexto semelhante</div><div class="table-like"><div class="table-head"><div>Escola</div><div>Renda (3 km)</div><div>Perfil PPI</div><div>Nota</div></div>'+rows+'</div>';
...
function scatterSvg(data,selectedKey,axis){
 var W=760,H=390,pad={l:66,r:24,t:28,b:58};
 var value=function(d){return axis==='ppi'?d.c.ppi:d.c.renda};
 var valid=data.filter(function(d){return Number.isFinite(value(d))&&Number.isFinite(d.y)});
 var xs=valid.map(value),ys=valid.map(function(d){return d.y});
 ...
 var xpad=(xmax-xmin)*.04||1,ypad=(ymax-ymin)*.06||1;xmin-=xpad;xmax+=xpad;ymin-=ypad;ymax+=ypad;
 function X(x){return pad.l+(x-xmin)/(xmax-xmin)*(W-pad.l-pad.r)}
 function Y(y){return H-pad.b-(y-ymin)/(ymax-ymin)*(H-pad.t-pad.b)}
 var html='<svg class="chart chart-scatter" viewBox="0 0 '+W+' '+H+'" role="img" aria-label="Contexto por desempenho">';
 html+='<line x1="'+pad.l+'" y1="'+(H-pad.b)+'" x2="'+(W-pad.r)+'" y2="'+(H-pad.b)+'" stroke="#cbd5e1"/><line x1="'+pad.l+'" y1="'+pad.t+'" x2="'+pad.l+'" y2="'+(H-pad.b)+'" stroke="#cbd5e1"/>';
 html+='<text x="'+(W/2)+'" y="'+(H-18)+'" text-anchor="middle" class="axis-label">'+(axis==='ppi'?'População PPI média no raio de 3 km':'Renda média no raio de 3 km')+'</text><text x="17" y="'+(H/2)+'" transform="rotate(-90 17 '+(H/2)+')" text-anchor="middle" class="axis-label">Desempenho</text>';
 valid.slice(0,500).forEach(function(d){var key=schoolKey(d.f),sel=key===selectedKey;html+='<circle class="point scatter-point'+(sel?' selected-point':'')+'" data-school-key="'+esc(key)+'" cx="'+X(value(d))+'" cy="'+Y(d.y)+'" r="'+(sel?10:4.5)+'" fill="'+(sel?'#f59e0b':'#2563eb')+'" stroke="'+(sel?'#0f172a':'#fff')+'" stroke-width="'+(sel?3:1)+'" opacity="'+(sel?1:.72)+'"><title>'+esc(schoolName(d.f))+' · '+(axis==='ppi'?'PPI '+people(value(d)):'Renda '+money(value(d)))+' · Nota '+fmt(d.y,1)+'</title></circle>'});
 if(selectedKey){html+='<text x="'+(W-24)+'" y="24" text-anchor="end" class="selected-label">● Escola selecionada</text>'}
...
function renderPhase4(){
 ...
 var html='<div class="hero"><div class="analysis-kicker">FASE 4 · CONTEXTO × DESEMPENHO</div><h3>'+esc(name)+' e perfil territorial</h3><div class="muted">Os valores territoriais são médias dos setores encontrados em um raio de 3 km da escola.</div></div>';
 html+='<div class="axis-switch"><span>Comparar no eixo X:</span><button class="btn '+(state.contextAxis==='renda'?'active':'')+'" id="axis-renda">Renda</button><button class="btn '+(state.contextAxis==='ppi'?'active':'')+'" id="axis-ppi">População PPI</button></div>';
 html+='<div class="cards">'+card('Escolas no modelo',fmt(model.n,0),'Escolas com nota e contexto de 3 km')+card('R² do modelo',model.r2===null?'—':fmt(model.r2*100,1)+'%','Variação do desempenho explicada pelas duas variáveis')+card('Observado',fmt(obs,1),'Nota da escola')+card('Esperado',fmt(expected,1),(res>=0?'<span class="badge good">+'+fmt(res,1)+' acima</span>':'<span class="badge bad">'+fmt(res,1)+' abaixo</span>'))+'</div>';
 html+='<div class="section-title">Relação entre contexto e desempenho</div>'+scatterSvg(data,schoolKey(selectedSchool),state.contextAxis);
 html+='<div class="info">Clique em qualquer ponto para selecionar a escola, aproximar o mapa e abrir os dados dela. O ponto amarelo representa a escola atualmente selecionada.</div>';
 ...
 html+='<div class="callout amber"><b>R² = '+(model.r2===null?'—':fmt(model.r2*100,1)+'%')+':</b> este valor é baixo. Significa que renda e PPI, sozinhos, explicam apenas uma pequena parte da variação observada nas notas. Isso é esperado em uma análise exploratória simples e reforça que o contexto socioeconômico não determina sozinho o desempenho.</div>';
 html+='<div class="callout amber"><b>Cuidado metodológico:</b> este modelo é exploratório. Ele não mede causalidade, não controla todas as características das escolas e não deve ser usado para ranquear unidades.</div>';
 html+='<div class="callout green"><b>O que investigar:</b> escolas com resíduo positivo podem ser pontos de partida para investigar práticas e condições de funcionamento. O resultado é um sinal para investigação, não uma explicação pronta.</div>';
 return html;
}
...
function loadAllContexts(){var arr=schoolFeatures.slice();var chain=Promise.resolve();arr.forEach(function(f){if(!analysisCache[schoolKey(f)])chain=chain.then(function(){var c=schoolContext(f);if(c)analysisCache[schoolKey(f)]=c});});return loadSetores().then(function(){return chain}).then(function(){prepareContextThresholds();return true})}
...
$('analysis-content').addEventListener('click',function(e){var axis=e.target.closest('#axis-renda,#axis-ppi');if(axis){state.contextAxis=axis.id==='axis-ppi'?'ppi':'renda';renderAnalysis();return}var point=e.target.closest('.scatter-point');if(point){var key=point.getAttribute('data-school-key'),f=schoolFeatures.find(function(x){return schoolKey(x)===key});if(f){state.etapa=norm(f.get('ETAPA'));state.rede=norm(f.get('REDE'));state.municipio=norm(f.get('NO_MUNICIPIO'));$('etapa').value=state.etapa;$('rede').value=state.rede;$('municipio').value=state.municipio;selectSchool(f,false);state.tab='1';openAnalysis('1')}}});
map.on('singleclick',function(evt){var hit=false;map.forEachFeatureAtPixel(evt.pixel,function(feature,layer){if(hit)return;if(layer===lyr_MetadePortugus_5||layer===lyr_MetadeMatemtica_4){if(visibleSchoolFeature(feature)){selectSchool(feature,true);hit=true}}else if(layer===setorRendaLayer||layer===setorPpiLayer){var p=feature.getProperties();$('analysis-title').textContent='Setor censitário';$('analysis-content').innerHTML='<div class="hero"><div class="analysis-kicker">CONTEXTO TERRITORIAL</div><h3>'+esc(norm(p.NM_MUN)||'Setor censitário')+'</h3><div class="muted">Código '+esc(norm(p.CD_SETOR))+'</div></div><div class="cards">'+card('Renda média',money(p.Renda),'Setor censitário')+card('Perfil PPI',ppiContextLabel(num(p.PPI)),'Concentração de população PPI')+'</div><div class="callout">O setor descreve o território de localização. Não representa, individualmente, todos os estudantes da escola.</div>';$('analysis-panel').classList.add('visible');document.querySelectorAll('.tab').forEach(function(b){b.classList.remove('active')});hit=true}}, {hitTolerance:6})});
